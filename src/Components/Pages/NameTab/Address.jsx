import React, { useState } from 'react'
import DatePicker from 'react-datepicker'
const Address
    = () => {
        const [showPage, setShowPage] = useState(false);

        return (
            <div id="name_Address" className="tab-pane fade">
                <div className="row px-2">
                    <div className="col-12 d-flex py-1 px-2" style={{ background: '#eee', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span>Address
                        </span>
                        <button className='btn btn-sm bg-green text-white py-0' onClick={() => setShowPage(!showPage)}><i className="fa fa-plus"></i></button>
                    </div>
                </div>
                <div className="row">
                    {
                        showPage ?
                            <div className="col-12">
                                <div className="row mt-3">
                                    <div className="col-12">
                                        <div className="text-field">
                                            <input type="text" required />
                                            <label>Address</label>
                                        </div>
                                    </div>
                                    <div className="col-2 mt-3">
                                        <div class="custom-control custom-radio pmd-radio custom-control-inline">
                                            <input type="radio" id="customRadioInline1" name="customRadio" class="custom-control-input" />
                                            <label class="custom-control-label pmd-radio-ripple-effect" for="customRadioInline1">Permanent</label>
                                        </div>
                                    </div>
                                    <div className="col-2 mt-3">
                                        <div class="custom-control custom-radio pmd-radio custom-control-inline">
                                            <input type="radio" id="customRadioInline2" name="customRadio" class="custom-control-input" />
                                            <label class="custom-control-label pmd-radio-ripple-effect" for="customRadioInline2">Temporary</label>
                                        </div>
                                    </div>
                                    <div className="col-2 mt-3">
                                        <div class="custom-control custom-radio pmd-radio custom-control-inline">
                                            <input type="radio" id="customRadioInline3" name="customRadio" class="custom-control-input" />
                                            <label class="custom-control-label" for="customRadioInline3">Old</label>
                                        </div>
                                    </div>
                                    <div className="col-2 mt-3">
                                        <div class="custom-control custom-radio pmd-radio custom-control-inline">
                                            <input type="radio" id="customRadioInline4" name="customRadio" class="custom-control-input" />
                                            <label class="custom-control-label" for="customRadioInline4">Alternate</label>
                                        </div>
                                    </div>

                                    <div className="col-1 mt-3">
                                        <div className="form-group">
                                            <label className="custom-control custom-checkbox    ">
                                                <input type="checkbox" className="custom-control-input" />
                                                <span className="custom-control-label" style={{ fontSize: '14px' }}>Current</span>
                                            </label>
                                        </div>
                                    </div>
                                    <div className="col-3 mt-3">
                                        <DatePicker
                                            dateFormat="MM/dd/yyyy HH:mm"
                                            timeInputLabel name='ArrivedDate'
                                            autoComplete="off"
                                            placeholderText="Date From"
                                        />
                                    </div>

                                    <div className="col-12 mt-3">
                                        <button className="btn btn-sm bg-green text-white" onClick={() => setShowPage(!showPage)}>Cancel</button>
                                        <button className="btn btn-sm bg-green text-white ml-2">Add Address</button>
                                    </div>
                                </div>
                            </div>
                            :
                            <div className="col-12">
                                <table className="table w-100">
                                    <tr>
                                        <th>#</th>
                                        <th>Address</th>
                                        <th>Address FROM</th>
                                        <th>TO</th>
                                        <th>ADDRESS TYPE</th>
                                        <th className='text-right'>ACTION</th>
                                    </tr>
                                    <tr>
                                        <td>1</td>
                                        <td>Bank ST Elizabeth NJ 25422 </td>
                                        <td>05-25-2020 </td>
                                        <td>05-28-2020</td>
                                        <td>Permanent</td>
                                        <td className='text-right'>
                                            <button className="btn btn-sm py-0 bg-green text-white" ><i className="fa fa-edit"></i></button>
                                            <button className="btn btn-sm py-0 bg-green text-white ml-1"><i className="fa fa-trash"></i></button>
                                        </td>
                                    </tr>
                                </table>
                            </div>
                    }
                </div>
            </div>
        )
    }

export default Address 
